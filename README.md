...
将 `.trae/skills/critical-skill/SKILL.md` 作为项目级 Trae skill 直接使用：

```bash
# 项目级（推荐）
mkdir -p .trae/skills/critical-skill
cp .trae/skills/critical-skill/SKILL.md .trae/skills/critical-skill/SKILL.md
```

Trae IDE 自动识别 `.trae/skills/<name>/SKILL.md` 目录结构，无需额外配置。
...
修改规则正文时，只编辑 `body.md`（唯一源），然后运行 `make sync` 重新生成 3 份平台文件。
